...
function injectStyles() {
    ...
    const css = `
        ...
        .bs-inv-school { display:flex; gap:12px; align-items:center; }
        .bs-inv-logo { width:56px; height:56px; object-fit:contain; border-radius:8px; background:#f1f5f9;
                       display:flex; align-items:center; justify-content:center; font-size:26px; color:#94a3b8; }
        .bs-inv-sch-name { font-size:19px; font-weight:800; color:#111827; line-height:1.2; }
        .bs-inv-sch-sub { font-size:.74rem; color:#6b7280; margin-top:2px; line-height:1.45; }
    ...
    wrap.innerHTML = `
        ...
        <div id="bs-inv-paper" class="bs-inv-paper" onclick="event.stopPropagation()">
            <div class="bs-inv-top">
                <div class="bs-inv-school">
                    ${logoHtml}
                    <div>
                        <div class="bs-inv-sch-name">${esc(sch?.name) || 'Sekolah'}</div>
                        <div class="bs-inv-sch-sub">${esc(schSub) || '&nbsp;'}</div>
                    </div>
                </div>
            ...
            <div class="bs-inv-grid">
                <div>
                    ...
                    <b>${esc(sch?.name) || '-'}</b><br>
                    <span style="color:#4b5563;">${sch?.address ? esc(sch.address) : '-'}<br>
